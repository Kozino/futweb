      ...
      <main className="fw-container py-8 sm:py-10">
        <div className="grid gap-4 lg:grid-cols-[1.35fr_0.65fr]">
          ...
          <aside className="space-y-4">
            ...
            <Card className="p-5"><h3 className="text-sm font-extrabold">Data confidence</h3><p className="mt-1 text-xs text-ink-500">A number is more useful when clubs can see how much evidence supports it.</p><ProgressBar className="mt-4" value={p.confidence.score} tone={p.confidence.score >= 65 ? 'trust' : 'gold'} showLabel /><div className="mt-3 space-y-2">{p.confidence.factors.map(f => <div key={f.label} className="flex items-center justify-between text-xs"><span className="text-ink-600">{f.label}</span><span className="font-bold">{f.detail}</span></div>)}</div></Card>
