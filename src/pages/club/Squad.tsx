...
import { Badge, Button, Card, Icon, Input, ProgressBar, Select, Tabs, toast } from '@/components/ui'
import { DEMO_PLAYERS, enrichPlayer } from '@/data/mock'

export default function Squad() {
  const squad = useMemo(() => DEMO_PLAYERS.slice(0, 10).map(enrichPlayer), [])
  ...
  return (
    <div>
      <PageHeader breadcrumb="Club workspace" icon="users" title="Squad"
        subtitle={`${squad.length} registered players · 250 seats on your plan`}
        actions={
          <>
            <Button variant="outline" icon="upload">Import CSV</Button>
            <Button icon="plus" onClick={() => toast({ tone: 'info', title: 'Invite sent', description: 'Players join by link and complete their own profile.' })}>
              Add player
            </Button>
          </>
        } />
                  ...
                  <td className="px-4 py-3">
                    <Link to={`/club/player/${p.id}`} className="flex items-center gap-2.5">
                      ...
                      <span>
                        ...
                        <span className="block text-2xs text-ink-400">{p.clubName}</span>
                      </span>
                    </Link>
                  </td>
