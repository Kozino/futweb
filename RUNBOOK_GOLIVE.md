...
11. **`supabase/migrations/0018_admin_monitor.sql`** — admin monitoring for Federation & developer access (NEW). Four admin-only, `security definer` reporting functions (`admin_federation_tree`, `admin_api_keys_view`, `admin_webhook_health`, `admin_platform_monitor`) gated on `is_admin()`, returning **sanitised** rows (API-key prefix only, never the secret/hash). Drives the new **Admin → Federation & API** page (`/admin/federation-api`) for monitoring academy groups, API-key usage, and webhook-delivery health.
